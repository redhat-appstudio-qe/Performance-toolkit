...
func New(t terminal.Terminal, cl client.Client, token string, interval time.Duration) *Gatherer {
	...
	prometheusClient := GetPrometheusClient(cl, token)
...
//nolint
func NewEmpty(cl client.Client, interval time.Duration) *Gatherer {
	g := &Gatherer{
		k8sClient:     cl,
		queryInterval: interval,
	}
...
func (g *Gatherer) StartGathering() chan struct{} {
	if len(g.mqueries) == 0 {
		klog.Infof("Metrics gatherer has no queries defined, skipping metrics gathering...")
		return nil
	}

	// ensure metrics are dumped if there's a fatal error
	//klog.AddPreFatalExitHook(g.PrintResults)
...
func (g *Gatherer) PrintResults() {
	for _, q := range g.mqueries {
		switch q.ResultType() {
		case "percentage":
			PrintPercentage(q.Name(), g.results[q.Name()])
		case "memory":
			PrintMemory(q.Name(), g.results[q.Name()])
		case "simple":
			PrintSimple(q.Name(), g.results[q.Name()])
		default:
			klog.Fatalf("invalid query", fmt.Errorf("query %s is missing a result type", q.Name()))
		}
	}
}

func PrintPercentage( name string, r aggregateResult) {
	avg := r.sum / float64(r.sampleCount)
	klog.Infof("Average %s: %.2f %%", name, avg*100)
	klog.Infof("Max %s: %.2f %%", name, r.max*100)
}

func PrintMemory( name string, r aggregateResult) {
	avg := r.sum / float64(r.sampleCount)
	klog.Infof("Average %s: %s", name, bytesToMBString(avg))
	klog.Infof("Max %s: %s", name, bytesToMBString(r.max))
}

func PrintSimple(name string, r aggregateResult) {
	avg := r.sum / float64(r.sampleCount)
