...
func GetPrometheusClient(cl client.Client, token string) prometheus.API {
	url, err := getPrometheusEndpoint(cl)
	if err != nil {
		klog.Fatalf("error creating client: failed to get prometheus endpoint", err)
	}
	httpClient, err := Client(url, token)
	if err != nil {
		klog.Fatalf("error creating client", err)
	}
